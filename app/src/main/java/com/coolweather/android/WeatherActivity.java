...
public class WeatherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        degressText = findViewById(R.id.degree_txt);
        ...
        if (weatherString != null) {
            Weather weather = Utility.handleWweatherResponse(weatherString);
            showWeatherInfo(weather);
        } else {
            String weatherId = getIntent().getStringExtra("weather_id");
                //第一次请求时肯定无数据，隐藏scrollview
        ...
        if (bingPic != null) {
            //这样判断有问题，永远只会加载第一张图，可以将bing的图片地址保存下来与下次的做比较，不一样就加载新的。
    ...
    public void requestWeather(final String weatherId) {
        ...
        HttpUtil.sendOkHttpRequest(weatherUrl, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                final Weather weather = Utility.handleWweatherResponse(responseText);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
